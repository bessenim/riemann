...
for g = 1:sg
    ...
    la_g(g) = 2; mu_g(g) = 1; ro_g(g) = 1*g*g;% параметры среды
end
...
dmin_g_i(1,1) = dmin_i(1); dmax_g_i(1,1) = 0;
    dmin_g_i(2,1) = 0; dmax_g_i(2,1) = dmax_i(1);            % Левая и правая границы области
...
sp_i = zeros(1,sdim);   sp_i(1) = 2*100+1;  sp_i(2) = 2*100+1;              % Количество узлов сетки-всегда нечетное
...
for p1 = 1:(sp_g_i(g,1)-1)
    for p2 = 1:(sp_g_i(g,2))
        u0_p1_p2_n(p1,p2,1) = -6;
        u0_p1_p2_n(p1,p2,2) = -1;
        u0_p1_p2_n(p1,p2,3) = 1;
        u0_p1_p2_n(p1,p2,4) = -3;
        u0_p1_p2_n(p1,p2,5) = 3;
    end
end
for p1 = sp_g_i(g,1)
    for p2 = 1:(sp_g_i(g,2))
        u0_p1_p2_n(p1,p2,1) = -2;
        u0_p1_p2_n(p1,p2,2) = 0;
        u0_p1_p2_n(p1,p2,3) = -0.5;
        u0_p1_p2_n(p1,p2,4) = -2.5;
        u0_p1_p2_n(p1,p2,5) = 3.5;
    end
end
...
for g = 1:sg
    K_g_p1_p2_jt{g} = squeeze(u_g_p1_p2_n_jt{g}(:,:,5,:))*0.1;
end
...
x_p2 = x_p{1,2};
K_p1_p2_jt = cat(1,K_g_p1_p2_jt{1},K_g_p1_p2_jt{2});
...
plot(x_p1, K_p1_p2_jt(:,(sp2-1)/2,20))
