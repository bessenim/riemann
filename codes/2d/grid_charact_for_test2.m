...
la = 2; mu = 1; ro = 1;                                                     % параметры среды
sn = 5;
...
dmin_i(2) = -2;  dmax_i(2) = 0;

dmin_g_i = zeros(sg,sdim); dmax_g_i = zeros(sg,sdim);
dmin_g_i(1,1) = dmin_i(1); dmax_g_i(1,1) = dmax_i(1);
dmin_g_i(1,2) = dmin_i(2); dmax_g_i(1,2) = dmax_i(2);
% dmin_g_i(1,1) = dmin_i(1); dmax_g_i(1,1) = 0;
%     dmin_g_i(2,1) = 0; dmax_g_i(2,1) = dmax_i(1);            % Ћева€ и права€ границы области
% dmin_g_i(1,2) = dmin_i(2); dmax_g_i(1,2) = dmax_i(2);
...
sp_i = zeros(1,sdim);   sp_i(1) = 2*30+1;  sp_i(2) = 2*30+1;              %  оличество узлов сетки-всегда нечетное
sp_g_i = zeros(sg,sdim);
sp_g_i(1,1) = sp_i(1);
sp_g_i(1,2) = sp_i(2);
% sp_g_i(1,1) = (sp_i(1)-1)/2 + 1;
%     sp_g_i(2,1) = (sp_i(1)-1)/2 + 1;
% sp_g_i(1,2) = sp_i(2);
%     sp_g_i(2,2) = sp_i(2);
...
%     indlambdge0_i(i,:) = find(squeeze(lambd_i_k(i,:)>=0));  lengthindlambdge0_i(i) = length(indlambdge0_i(i,:));
    ...
    LGT0_i_m_n(i,:,:) = L_i_m_n(i,find(squeeze(lambd_i_k(i,:)>0)),:);
    LLE0_i_m_n(i,:,:) = L_i_m_n(i,find(squeeze(lambd_i_k(i,:)<=0)),:);
    LLT0_i_m_n(i,:,:) = L_i_m_n(i,find(squeeze(lambd_i_k(i,:)<0)),:);
    for k=1:sn
...
deltat = min(hmin_i./transpose(max(abs(lambd_i_k),[],2))) * 0.25;
Ntau = fix(T/deltat);
...
%   bound = 1   g = 1
    for p1 = 1
        for p2 = 1:(sp_g_i(g,2)-1)
            ...
            A = squeeze(LGT0_i_m_n(1,:,:)) * (eye(sn) - squeeze(C_p1_p2_m_n{g}(p1,p2, :, :)));
            C = squeeze(LLE0_i_m_n(1,:,:)) * squeeze(C_p1_p2_m_n{g}(p1,p2, :, :));
            B = cat(1,C,A);
            R_g_p1_p2_m_n{g,p1,p2} = inv(B);
        end
    end
%   bound = 2   g = 1
    for p1 = 1
    ...
    for p1 = 2:(sp_g_i(g,1)-1)
        for p2 = sp_g_i(g,2)
            %   ѕрозрачные граничные услови€
            A = squeeze(LLT0_i_m_n(2,:,:)) * (eye(sn) - squeeze(C_p1_p2_m_n{g}(p1,p2, :, :)));
...
%   bound = 3   g = 1    
    for p1 = sp_g_i(g,1)
        for p2 = 2:(sp_g_i(g,2))
            %   ѕрозрачные граничные услови€
            A = squeeze(LLT0_i_m_n(1,:,:)) * (eye(sn) - squeeze(C_p1_p2_m_n{g}(p1,p2, :, :)));
            C = squeeze(LGE0_i_m_n(1,:,:)) * squeeze(C_p1_p2_m_n{g}(p1,p2, :, :));
            B = cat(1,C,A);
            R_g_p1_p2_m_n{g,p1,p2} = inv(B);
        end
    end
...
%   bound = 4   g = 1
    ...
    for p1 = 2:(sp_g_i(g,1)-1)
        for p2 = 1
            %   ѕрозрачные граничные услови€
            A = squeeze(LGT0_i_m_n(2,:,:)) * (eye(sn) - squeeze(C_p1_p2_m_n{g}(p1,p2, :, :)));
            C = squeeze(LLE0_i_m_n(2,:,:)) * squeeze(C_p1_p2_m_n{g}(p1,p2, :, :));
            B = cat(1,C,A);
            R_g_p1_p2_m_n{g,p1,p2} = inv(B);
        end
    end
...
for p2 = ((sp_g_i(g,2)-1)/2 + 1):sp_g_i(g,2)
    for p1 = 1:(sp_g_i(g,1))
        u0_p1_p2_n(p1,p2,1) = -4;
        u0_p1_p2_n(p1,p2,2) = -3;
        u0_p1_p2_n(p1,p2,3) = 1;
...
% for p2 = 1:(sp_g_i(g,2))
%     u0_p1_p2_n(1,p2,1) = -4;
%     u0_p1_p2_n(1,p2,2) = -2.5;
%     u0_p1_p2_n(1,p2,3) = 1.5;
%     u0_p1_p2_n(1,p2,4) = 0;
%     u0_p1_p2_n(1,p2,5) = -0.5;
% end
...
for g = sg:-1:1
    ...
    g = 1;
    amplitude_p1w_sp2{g}(sp_g_i(g,1), 1) = 0;
%     g = 2;
%     amplitude_p1w_sp2{g}(1, 1) = 0;
...
for jt=2:sjt
    for point = 2:deltaNinput+1
        ...
        for p1 = 1
            for p2 = 1:(sp_g_i(g,2)-1)
                r1_m =squeeze(LLE0_i_m_n(1,:,:))*squeeze(unext_g_p1_p2_n{g}(p1,p2,:));
                r2_m = squeeze(LGT0_i_m_n(1,:,:)) * zeros(sn,1);
...
%   bound = 2   g = 1
    ...
    for p1 = 2:(sp_g_i(g,1)-1)
        for p2 = sp_g_i(g,2)
                ...
                r2_m = squeeze(LLT0_i_m_n(2,:,:)) * zeros(sn,1);
                rx_m = cat(1,r1_m,r2_m); 
                unext_g_p1_p2_n{g}(p1,p2,:) = R_g_p1_p2_m_n{g,p1,p2}*rx_m;
            end
        end
...
%   bound = 3   g = 1    
    for p1 = sp_g_i(g,1)
        for p2 = 2:(sp_g_i(g,2))
                r1_m =squeeze(LGE0_i_m_n(1,:,:))*squeeze(unext_g_p1_p2_n{g}(p1,p2,:));
                r2_m = squeeze(LLT0_i_m_n(1,:,:)) * zeros(sn,1);
                rx_m = cat(1,r1_m,r2_m); 
                unext_g_p1_p2_n{g}(p1,p2,:) = R_g_p1_p2_m_n{g,p1,p2}*rx_m;
            end
    end
...
%   bound = 4   g = 1
    ...
    for p1 = 2:(sp_g_i(g,1)-1)
        for p2 = 1
                r1_m =squeeze(LLE0_i_m_n(2,:,:))*squeeze(unext_g_p1_p2_n{g}(p1,p2,:));
                r2_m = squeeze(LGT0_i_m_n(2,:,:)) * zeros(sn,1);
                rx_m = cat(1,r1_m,r2_m); 
                unext_g_p1_p2_n{g}(p1,p2,:) = R_g_p1_p2_m_n{g,p1,p2}*rx_m;
            end
    end
...
for g = 1:sg
    K_g_p1_p2_jt{g} = squeeze(u_g_p1_p2_n_jt{g}(:,:,1,:));
end
...
x_p1 = x_p{1,1};   
x_p2 = x_p{1,2};
K_p1_p2_jt = K_g_p1_p2_jt{1}*0.1;
[sp1,sp2,sj] = size(K_p1_p2_jt);

x1min = min(x_p1);                       x1max = max(x_p1);
x2min = min(x_p2);                       x2max = max(x_p2);

Kmin = min(min(min(K_p1_p2_jt, [], 1), [], 2));   Kmax = max(max(max(K_p1_p2_jt, [], 1), [], 2));
